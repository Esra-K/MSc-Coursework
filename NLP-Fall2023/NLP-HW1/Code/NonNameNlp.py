...
def process_string(s, window_size=10, ngram_max_n=3):
    global ngram_relations, frequencies, english_names, stemmer, lemmatizer  #, start_time
    temp = unidecode(s)
    temp = temp.lower()
...
""" added here """
frequencies_list = sorted(frequencies, key=frequencies.get, reverse=True)
with open("../Garbage/NonNameFrequencies.txt", "w+") as f:
    for item in frequencies_list:
        f.write(" ".join([item, str(frequencies[item]), "|"]))
""" end of added here"""
...
for i, name in enumerate(names):
    ...
    if degree[name] > 5 and len(name.strip().split(" ")) == 1:
...
tsne = TSNE(n_components=2, random_state=1377)
