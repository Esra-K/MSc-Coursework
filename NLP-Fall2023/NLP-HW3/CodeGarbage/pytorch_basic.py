...
def create_tensor_of_pi(M, N):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x = torch.tensor([[3.14 for j in range(N)] for i in range(M)]).type(torch.FloatTensor)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  l = [pointer for pointer in range(start, stop) if pointer % 10 == 0]
  if len(l) > 0:
...
def slice_assignment_practice(x):
  ...
  # x[:2, 0].copy_(torch.tensor([[0] * 2], dtype=torch.int))
  # x[:2, 1].copy_(torch.tensor([[1] * 2], dtype=torch.int))
  # x[:2, 2:6].copy_(torch.tensor([[[2] * 4] * 2], dtype=torch.int))
  # x[2:4, 0:3:2].copy_(torch.tensor([[[3] * 2] * 2], dtype=torch.int))
  # x[2:4, 1:4:2].copy_(torch.tensor([[[4] * 2] * 2], dtype=torch.int))
  # x[2:4, 4:6].copy_(torch.tensor([[[5] * 2] * 2], dtype=torch.int))
...
def count_negative_entries(x):
  ...
  # Replace "pass" statement with your code
  mask = (x < 0)
  neg_subarray = x[mask].reshape(-1) # reshape(-1) flattens the array
  num_neg = neg_subarray.shape[0]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_neg
...
def reshape_practice(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone().detach()
  y = torch.t(y.view(2, 3, 4).transpose(1, 2).reshape(8, 3))
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  y[list(range(y.shape[0])), row_min_idxs] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  z = torch.rand((x.shape[0], x.shape[1], y.shape[2])).to(x)
  if use_loop:
